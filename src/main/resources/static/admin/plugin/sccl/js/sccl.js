sccl = {
	changeSkin: function (skin) {
		...
		var hrefRes = hrefStr.substring(0, hrefStr.lastIndexOf('skin/')) + 'skin/' + skin + '/skin.css';
		$(window.frames.document).contents().find("#layout-skin").attr("href", hrefRes);
		window.localStorage.scclui_skin = skin;
		return false;
	},
	/*获取LocalStorage中的皮肤*/
	getSkinByLocalStorage: function () {
		var skin = window.localStorage.scclui_skin;
		if(skin == null || skin == "") {
			skin = "qingxin";
		}
	...
	initMenu: function (menu) {
		for(var i = 0; i < menu.length; i++) {
			var item = menu[i];
			var str = '';
			if(!item.parentId) { // parentId == null =>模块
				str = '<li class="menu-item" id="menu-item-' + item.privilegeId + '"></li>'
				$(".side-menu").append(str);
				str = "<a href=''><i class='icon-font '>" + item.icon + "</i><span>" + item.name + "</span><i class='icon-font icon-right'>&#xe60b;</i></a>";
				$('#menu-item-' + item.privilegeId).append(str);
			} else { // parentId != null =>HTML
				var child = $('#menu-item-child-' + item.parentId);
				if(child.length == 0) { // 子菜单不存在
					str = "<ul class='menu-item-child' id='menu-item-child-" + item.parentId + "'></ul>"
					$('#menu-item-' + item.parentId).append(str);
					child = $('#menu-item-child-' + item.parentId);
				}
...
/*循环菜单*/
function initMenu2(menu, parent) {
	for(var i = 0; i < menu.length; i++) {
		var item = menu[i];
		var str = "";
		try {
...
//	$(window.frames.document).contents().find("#layout-skin").attr("href", hrefRes);
//	//	console.log(v);
//	//	window.localStorage.scclui-skin = v;
//});